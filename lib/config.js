...
function getConfig(dirname, options) {
    ...
    buildConfig.dist = options.outputDirectory || 'sea-modules';
    buildConfig.gzipAll = !!options.gzipAll;
...
function gzipConfig(buildConfig) {
    var dir = buildConfig.gzipAll ? buildConfig.dist : buildConfig.dist + '/' + buildConfig.src;
    return {
        options : {
            mode : 'gzip'
        },
